...
function ClientsPage() {
  ...
  return (
    <div className="p-4">
      ...
      <p className="text-right mb-4">إجمالي العملاء: <strong>{clients.length}</strong></p>
        ...
        return (
          ...
          >
            ...
            <div className="flex items-center flex-wrap gap-3">
              ...
              {client.code && <span className="bg-blue-600 text-white px-2 py-1 rounded">{client.code}</span>}
            </div>
          </div>
        );
      }) : (
        <div className="text-center text-gray-500 mt-10">لا يوجد عملاء لعرضهم أو لا توجد نتائج للبحث.</div>
      )}
                  ...
                  <tbody>
                    {selectedClient.orders.map((order, idx) => (
                      <tr key={idx} className="text-center">
                        <td className="px-3 py-2 border">{order.name || 'غير محدد'}</td>
                        <td className="px-3 py-2 border">{order.cost?.toFixed(2)}</td>
                        <td className="px-3 py-2 border">{order.profit?.toFixed(2)}</td>
                        <td className="px-3 py-2 border">{order.total?.toFixed(2)}</td>
                        <td className="px-3 py-2 border">{order.date ? new Date(order.date).toLocaleDateString() : 'غير محدد'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              </div>
            ) : <p className="text-gray-500">لا يوجد أوردرات لهذا العميل حتى الآن.</p>}
